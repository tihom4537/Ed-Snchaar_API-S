...
{   private $headers;
    ...
    //         $stmt->bindParam(':school_id', $school_id, PDO::PARAM_STR); // Add ':' before school_id
    ...
    public function updateProgress()
{
    ...
    if (empty($class_id) || empty($school_id) || empty($subject) || empty($subject_content) || $status === '') {
        echo "Invalid parameters. Please provide all required parameters.";
        return;
    }
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                echo "No matching records found. Syllabus not updated.";
            }
        } else {
            echo "Error updating syllabus.";
        }
    } catch (PDOException $e) {
        echo "Error: " . $e->getMessage();
    }
