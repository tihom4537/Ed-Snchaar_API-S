...
class AttendanceController
{
    ...
    public function handleRequest()
    {
        ...
        if ($this->validateApiKey($decryptedApiKey)) {
            ...
            if ($method == 'GET') {
                ...
                if (empty($classId) || empty($date) || empty($month) || empty($year)) {
                    http_response_code(400); // Bad Request
                    echo json_encode(['error' => "class_id, date, month, and year parameters are required."]);
                    return;
                }
                ...
                foreach ($usernames as $username) {
                    $attendance = $this->fetchAttendance($username, $date, $month, $year);
    ...
    public function fetchAttendance($username, $date, $month, $year)
    {
        try {
