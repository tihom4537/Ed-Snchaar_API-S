...
class AttendanceController
{
    ...
    if ($this->validateApiKey($decryptedApiKey)) {
        ...
        if (empty($schoolId) || empty($month) || empty($year)) {
            http_response_code(400); // Bad Request
            echo json_encode(['error' => "school_id, month, and year parameters are required."]);
            return;
        }

        $usernames = $this->fetchUsernames($schoolId);
    ...
    private function fetchUsernames($schoolId)
    {
        try {
            $sql = "SELECT username FROM users WHERE school_id = :school_id";

            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':school_id', $schoolId, PDO::PARAM_STR);
            $stmt->execute();
