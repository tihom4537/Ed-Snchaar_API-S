...
class ComplaintStatusFetch
{
    ...
    public function fetchComplaints() // Corrected function name
    {
        ...
        $complaintQuery = "SELECT * FROM notice WHERE school_id = :school_id and class_id = :class_id ";
        $complaintStmt = $this->conn->prepare($complaintQuery);
        $complaintStmt->bindParam(':school_id', $school_id, PDO::PARAM_STR);
        $complaintStmt->bindParam(':class_id', $class_Id, PDO::PARAM_STR);
        $complaintStmt->execute();
        $complaints = $complaintStmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Check if complaints were found
        if (empty($complaints)) {
            echo json_encode(['message' => "No complaints raised for the specified school_id."]);
        } else {
