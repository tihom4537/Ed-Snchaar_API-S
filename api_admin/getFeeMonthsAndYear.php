...
class GetAttendanceMonths {
    ...
    public function getSchoolUsers($school_id) {
        ...
        $stmt->bindParam(':schoolId', $school_id, PDO::PARAM_STR);
        $stmt->execute();
        $usernames = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $this->getFeeMonths($usernames);
    }

    public function getFeeMonths($usernames) {
        $monthsByYear = [];

        foreach($usernames as $username) {
            $sql = "SELECT DISTINCT month, year FROM fees WHERE username = :username";
            $stmt = $this->conn->prepare($sql);
