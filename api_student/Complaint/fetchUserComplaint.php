...
class ComplaintStatusFetch
{
    ...
    public function fetchComplaints() // Corrected function name
    {
        ...
        if (empty($username)) {
            echo json_encode(['error' => "username parameter is required."]); // Return JSON error response
            return;
        } else {
            // Use prepared statements to prevent SQL injection
            $complaintQuery = "SELECT * FROM complaints WHERE username = :username";
            $complaintStmt = $this->conn->prepare($complaintQuery);
            $complaintStmt->bindParam(':username', $username, PDO::PARAM_STR);
